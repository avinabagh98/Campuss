...
import { View, TouchableOpacity, Text, StyleSheet, Platform, Animated, StatusBar } from 'react-native';
import Svg, { Path } from 'react-native-svg';
import { FontAwesomeIcon } from '@fortawesome/react-native-fontawesome'
import { faHome, faUser, faBarcode } from '@fortawesome/free-solid-svg-icons';
...
function MyTabBar({ state, descriptors, navigation }) {
    ...
    useEffect(() => {
        ...
        const floatingTo = typeof center === 'number' ? center - 32 : tabW * focusedIndex + (tabW / 2) - 32;
                            ...
                            <FontAwesomeIcon
                                icon={
                                    state.routes[state.index].name === 'Home'
                                        ? faHome
                                        : state.routes[state.index].name === 'Scan'
                                            ? faBarcode
                                            : faUser
                                }
                                size={20}
